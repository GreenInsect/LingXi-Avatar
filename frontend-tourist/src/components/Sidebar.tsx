...
interface NavEntry { id: PageId; icon: string; label: string }
  ...
  return (
    <>
      {open && (
        <div
          ...
          style={{
            ...
            background: 'rgba(26,15,10,0.25)', backdropFilter: 'blur(2px)',
      ...
      <div style={{
        ...
        borderRight: '1px solid var(--border)', zIndex: 150,
        ...
        <div style={{ padding: '20px 16px' }}>
          ...
          {NAV.map(item => (
            <button key={item.id} onClick={() => onNavigate(item.id)} style={{
              ...
              fontSize: 13, letterSpacing: 0.3, transition: 'all 0.18s',
            }}
              onMouseEnter={e => { if (activePage !== item.id) (e.currentTarget as HTMLButtonElement).style.background = 'var(--gold-pale)' }}
              onMouseLeave={e => { if (activePage !== item.id) (e.currentTarget as HTMLButtonElement).style.background = 'transparent' }}
            >
