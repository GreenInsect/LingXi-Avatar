...
/* ── Mini SVG avatar ────────────────────────────────────── */
function AvatarFace({ emotion = 'happy', size = 52, speaking = false }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 160 160" style={{ display:'block' }}>
      <defs>
        <radialGradient id="sg2" cx="40%" cy="35%"><stop offset="0%" stopColor="#f7d0a8"/><stop offset="100%" stopColor="#e8a878"/></radialGradient>
        <linearGradient id="bg2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#4a8c6e"/><stop offset="100%" stopColor="#2d6b52"/></linearGradient>
      </defs>
      {/* Body */}
...
function Bubble({ msg }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:isUser?'row-reverse':'row', gap:6, animation:'fadeUp 0.3s ease' }}>
      <div style={{ width:26, height:26, borderRadius:'50%', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12,
        background:isUser?'linear-gradient(135deg,var(--gold),var(--gold-light))':'linear-gradient(135deg,var(--jade),var(--jade2))' }}>
        {isUser?'👤':'🌸'}
      </div>
      <div style={{ maxWidth:'80%' }}>
        <div style={{
          padding:'8px 12px', fontSize:13, lineHeight:1.65, whiteSpace:'pre-wrap', wordBreak:'break-word',
          borderRadius: isUser?'14px 4px 14px 14px':'4px 14px 14px 14px',
          background:isUser?'linear-gradient(135deg,rgba(61,122,94,0.82),rgba(61,122,94,0.65))':'rgba(255,252,245,0.95)',
          color:isUser?'white':'var(--ink)',
          border:isUser?'none':'1px solid rgba(201,168,76,0.2)',
          boxShadow:'0 2px 8px rgba(0,0,0,0.06)',
        }}>{msg.content}</div>
        <div style={{ fontSize:9, color:'rgba(26,15,10,0.35)', marginTop:3, textAlign:isUser?'right':'left', padding:'0 4px' }}>{time}</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ display:'flex', gap:6, animation:'fadeIn 0.2s ease' }}>
      ...
      <div style={{ padding:'10px 14px', borderRadius:'4px 14px 14px 14px', background:'rgba(255,252,245,0.95)', border:'1px solid rgba(201,168,76,0.2)', display:'flex', gap:4 }}>
        {[0,.15,.3].map((d,i)=><div key={i} style={{ width:5, height:5, borderRadius:'50%', background:'var(--jade)', animation:`pulse 1.1s ease infinite`, animationDelay:`${d}s` }}/>)}
      </div>
    </div>
  )
}
...
/* ── Quick questions ────────────────────────────────────── */
const QUICK = ['景区有什么好玩的？','门票多少钱？','推荐一条游览路线','九龙灌浴几点表演？','祥符禅寺历史介绍','灵山梵宫怎么参观？']

/* ── Main floating avatar component ────────────────────── */
export default function FloatingAvatar({ open, onToggle }) {
  ...
  const handleSend = useCallback(async (customText, inputType = 'text') => {
    ...
    try {
      ...
      if (res?.avatar_emotion) setEmotion(res.avatar_emotion)
    } catch {}
  ...
  if (!open) {
    return (
      <div
        style={{
          position:'fixed', right:28, bottom:28, zIndex:500,
          cursor:'pointer',
        }}
        onClick={onToggle}
      >
      ...
      <div style={{
        ...
        <div style={{ position:'relative', flexShrink:0 }}>
          <div style={{
            ...
            <AvatarFace emotion={emotion} size={44} speaking={speaking || loading} />
          </div>
          {speaking && (
        ...
        {/* Emotion */}
        <div style={{ fontSize:18, transition:'all 0.4s' }} title={EMOTIONS[emotion]?.label}>{EMOTIONS[emotion]?.emoji}</div>
        ...
        <button
          ...
          onMouseEnter={e => e.currentTarget.style.background='rgba(26,15,10,0.14)'}
          onMouseLeave={e => e.currentTarget.style.background='rgba(26,15,10,0.07)'}
        >✕</button>
      </div>
          ...
          {QUICK.map(q => (
            <button
              ...
              style={{
                ...
                fontSize:11, color:'var(--ink2)', cursor:'pointer', whiteSpace:'nowrap',
                transition:'all 0.18s',
              }}
              ...
              onMouseLeave={e => { e.currentTarget.style.background='rgba(255,252,245,0.8)'; e.currentTarget.style.color='var(--ink2)'; e.currentTarget.style.borderColor='var(--border)' }}
            >{q}</button>
          ))}
        </div>
      )}
      ...
      <div style={{ padding:'10px 12px', borderTop:'1px solid var(--border)', background:'rgba(250,245,236,0.95)', flexShrink:0 }}>
        <div style={{ display:'flex', gap:8, alignItems:'flex-end' }}>
          ...
          <textarea
            ref={textareaRef}
            value={text}
            onChange={e => { setText(e.target.value); e.target.style.height='auto'; e.target.style.height=Math.min(e.target.scrollHeight,72)+'px' }}
