...
export default function Navbar({ activePage, onNavigate, onMenuToggle, sidebarOpen }) {
  return (
    <header style={{
      height: 'var(--nav-h)',
      ...
      boxShadow: '0 1px 16px rgba(26,15,10,0.06)',
      flexShrink: 0,
    }}>
      {/* Hamburger (mobile) */}
      <button
        ...
        style={{
          ...
          background: sidebarOpen ? 'var(--gold-pale)' : 'transparent',
          transition: 'all 0.2s',
        }}
      >
        {[0, 1, 2].map(i => (
        ...
        {NAV_ITEMS.map(item => (
          <button
            ...
            style={{
              ...
              border: activePage === item.id ? '1px solid rgba(201,168,76,0.35)' : '1px solid transparent',
              transition:'all 0.2s', letterSpacing:0.5,
            }}
            onMouseEnter={e => { if(activePage!==item.id) { e.currentTarget.style.background='var(--gold-pale)'; e.currentTarget.style.color='var(--gold)' }}}
            onMouseLeave={e => { if(activePage!==item.id) { e.currentTarget.style.background='transparent'; e.currentTarget.style.color='var(--ink2)' }}}
          >
            {item.label}
          </button>
        ))}
