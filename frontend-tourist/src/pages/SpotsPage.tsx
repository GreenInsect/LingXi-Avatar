...
function SpotCard({ spot, index, active, onClick }: SpotCardProps) {
  return (
    <div onClick={onClick} style={{
      ...
      onMouseEnter={e => { if (!active) (e.currentTarget as HTMLDivElement).style.boxShadow = 'var(--shadow)' }}
    ...
    >
      ...
      <div style={{ marginTop: 12, fontSize: 11, color: 'var(--jade)', display: 'flex', alignItems: 'center', gap: 4 }}>
        <span>⏰</span>
        <span style={{ overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{spot.openTime.split('\n')[0]}</span>
      </div>
    </div>
  )
}
