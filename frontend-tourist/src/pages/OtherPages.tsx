import { nianhewan, tickets, tips, dining, history as historyData } from '../data/lingshan'
import type { NianheWanSpot, Ticket, Tip, DiningItem, HistoryEvent } from '../types'
    ...
    <div style={{ height: '100%', overflowY: 'auto' }}>
      <div style={{
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ fontSize: 12, color: 'var(--jade)', letterSpacing: 3, marginBottom: 12, textTransform: 'uppercase' }}>Nianhewan Zen Town</div>
          <h2 style={{ fontFamily: "'Ma Shan Zheng',cursive", fontSize: 42, color: 'var(--ink)', letterSpacing: 4, marginBottom: 12 }}>拈花湾禅意小镇</h2>
          <p style={{ fontSize: 14, color: 'rgba(26,15,10,0.6)', maxWidth: 560, margin: '0 auto', lineHeight: 2 }}>
            与灵山胜境比邻而居，以"禅意·慢生活"为核心，融合禅意建筑、自然花海、非遗手作与特色演艺，白天花海漫步，夜间灯光秀如梦如幻。
          </p>
        </div>
      </div>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 28px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 20 }}>
          {nianhewan.map((item, i) => <NianheWanCard key={item.id} item={item} index={i} />)}
        </div>
      </div>
      <div style={{ height: 60 }} />
    </div>
...
function NianheWanCard({ item, index }: { item: NianheWanSpot; index: number }) {
  return (
    <div style={{
      background: 'rgba(255,252,245,0.9)', borderRadius: 14, border: '1px solid var(--border)',
      padding: '22px', animation: `fadeUp 0.4s ease ${index * 0.07}s both`, transition: 'all 0.22s',
    }}
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '40px 28px' }}>
        <PageTitle title="游览指南" sub="Visitor Guide" />

        <SectionBlock title="🎫 门票价格" color="var(--gold)">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 14 }}>
            {tickets.map((t: Ticket) => (
              <div key={t.type} style={{ background: 'rgba(255,252,245,0.9)', borderRadius: 12, border: '1px solid var(--border)', padding: '18px' }}>
                ...
                <div style={{ fontSize: 20, color: 'var(--gold)', fontWeight: 700, marginBottom: 8 }}>{t.price}</div>
        ...
        <SectionBlock title="⏰ 开放时间" color="var(--jade)">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            {[
              { icon: '☀️', title: '夏季', content: '4月-10月 08:00–18:00' },
              { icon: '❄️', title: '冬季', content: '11月-3月 08:30–17:00' },
            ].map(item => (
              <div key={item.title} style={{ background: 'rgba(255,252,245,0.9)', borderRadius: 12, border: '1px solid var(--border)', padding: '16px' }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 6 }}>
                  <span style={{ fontSize: 18 }}>{item.icon}</span>
                  <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink)' }}>{item.title}</span>
                </div>
                <div style={{ fontSize: 13, color: 'rgba(26,15,10,0.6)', lineHeight: 1.7 }}>{item.content}</div>
              </div>
            ))}
          </div>
        ...
        <SectionBlock title="🍜 餐饮推荐" color="var(--red)">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(250px,1fr))', gap: 14 }}>
            {dining.map((d: DiningItem) => (
        ...
        <SectionBlock title="💡 实用贴士" color="var(--jade)">
...
export function HistoryPage() {
  return (
    <div style={{ height: '100%', overflowY: 'auto' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '40px 28px' }}>
        <PageTitle title="历史文化" sub="History & Culture" />

        <SectionBlock title="🕰️ 千年发展历程">
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {historyData.map((h: HistoryEvent, i: number) => (
        ...
        <SectionBlock title="🏛️ 文化内涵">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            {CULTURE_SECTIONS.map((c, i) => (
