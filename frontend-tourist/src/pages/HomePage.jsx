  ...
  return (
    <div style={{ height:'100%', overflowY:'auto' }}>
      ...
      <section style={{
        minHeight:'70vh',
        background:`
          radial-gradient(ellipse 90% 60% at 50% 100%, rgba(61,122,94,0.18) 0%, transparent 70%),
        ...
        <div style={{ textAlign:'center', position:'relative', zIndex:2, animation:'fadeUp 0.7s ease' }}>
          ...
          <h1 style={{
            ...
            color:'var(--ink)', letterSpacing:8, lineHeight:1,
            marginBottom:16,
          ...
          <p style={{ fontSize:15, color:'var(--ink2)', lineHeight:2, maxWidth:580, margin:'0 auto 36px', letterSpacing:0.3 }}>
            坐落于江苏省无锡市太湖西北部马山镇，占地约30万平方米，三山环抱，
            太湖碧波，是佛教文化、自然山水与人文艺术的完美融合之地。
          </p>
          <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap', marginBottom:36 }}>
            {HERO_FEATURES.map(f => (
              <div key={f.text} style={{
                padding:'8px 18px', borderRadius:24,
                background:'rgba(255,252,245,0.7)', backdropFilter:'blur(8px)',
          ...
          <div style={{ display:'flex', gap:14, justifyContent:'center' }}>
            <button
              ...
              onMouseLeave={e => { e.currentTarget.style.transform='none'; e.currentTarget.style.boxShadow='0 4px 20px rgba(201,168,76,0.4)' }}
            >探索景点</button>
            <button
              ...
              style={{
                ...
                color:'var(--jade)', fontSize:14, letterSpacing:1,
                border:'1px solid var(--jade2)',
                cursor:'pointer', transition:'all 0.2s',
              }}
              onMouseEnter={e => { e.currentTarget.style.background='var(--jade-pale)' }}
      ...
      <section style={{ padding:'60px 32px', maxWidth:1200, margin:'0 auto' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))', gap:20, marginTop:32 }}>
          {featured.map((spot, i) => (
            <SpotCard key={spot.id} spot={spot} index={i} onClick={() => onNavigate('spots')} />
          ))}
        </div>
        <div style={{ textAlign:'center', marginTop:32 }}>
          <button
            onClick={() => onNavigate('spots')}
            style={{ padding:'10px 28px', borderRadius:24, border:'1px solid var(--border)', background:'transparent', color:'var(--jade)', fontSize:13, cursor:'pointer', transition:'all 0.2s' }}
            onMouseEnter={e => e.currentTarget.style.background='var(--jade-pale)'}
            onMouseLeave={e => e.currentTarget.style.background='transparent'}
          >查看全部景点 →</button>
        </div>
          ...
          {historyData.slice(0,4).map((h, i) => (
            <div key={i} style={{ display:'flex', gap:20, alignItems:'flex-start', paddingBottom:24, animation:`fadeUp 0.4s ease ${i*0.1}s both` }}>
              <div style={{ display:'flex', flexDirection:'column', alignItems:'center', flexShrink:0 }}>
                ...
                {i < historyData.slice(0,4).length-1 && <div style={{ width:1, flex:1, minHeight:24, background:'var(--border)', marginTop:4 }} />}
              </div>
...
function SpotCard({ spot, index, onClick }) {
  return (
    <div
      onClick={onClick}
      style={{
        background:'rgba(255,252,245,0.88)', borderRadius:14,
    ...
    >
      <div style={{
        height:100, display:'flex', alignItems:'center', justifyContent:'center', fontSize:52,
        background:`linear-gradient(135deg, var(--gold-pale), var(--jade-pale))`,
